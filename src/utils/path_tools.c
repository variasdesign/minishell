...
/*   Updated: 2026/01/22 19:28:05 by varias-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	char	*check_current_dir(char *exec)
{
	...
	{
		pwd = getcwd(NULL, 0);
		full_path = assemble_path(exec, pwd);
		if (!full_path)
			return (NULL);
...
static char	*valid_exec(char *exec, char **path_env)
{
	...
	if (path_env && path_env[i])
	{
		...
		{
			full_path = assemble_path(exec, path_env[i]);
			if (!full_path)
				return (NULL);
			if (!access(full_path, R_OK | X_OK))
				return (full_path);
...
static t_bool	period_and_empty_check(char *args)
{
	if (!*args || (ft_strlen(args) == 1 && !ft_strncmp(args, ".", 1))
...
int	get_exec_path(t_cmd *cmd, t_list *env_list)
{
	...
	if (cmd->args[0] && !period_and_empty_check(cmd->args[0]))
	{
